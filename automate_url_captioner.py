...
# 1. Load the pretrained processor and model (Blanks filled in!)
print("Loading model...")
processor = AutoProcessor.from_pretrained("Salesforce/blip-image-captioning-base")
model = BlipForConditionalGeneration.from_pretrained("Salesforce/blip-image-captioning-base")

# 2. URL of the page to scrape
...
# 4. Parse the page and find images
soup = BeautifulSoup(response.text, "html.parser")
img_elements = soup.find_all("img")
print(f"Found {len(img_elements)} <img> tags")

# 5. Process images and write to captions.txt
with open("captions.txt", "w", encoding="utf-8") as caption_file:
    for idx, img_element in enumerate(img_elements, start=1):
        ...
        try:
            ...
            # Skip very small images (like tiny icons)
...
print("\nDone! Check captions.txt for the results.")
